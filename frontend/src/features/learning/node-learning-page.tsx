...
export function NodeLearningPage() {
  ...
  const beginExam = useMutation({mutationFn: () => learningApi.startAttempt(nodeId, crypto.randomUUID()), onSuccess: row=>{setParams({attempt:row.id});setTab('exam')}, onError:(e:Error)=>toast.error(e.message)})
