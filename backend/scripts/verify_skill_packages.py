...
from app.services.skill_package import (  # noqa: E402
    ...
    is_official_skill_package_path,
...
def frontmatter_metadata_version(text: str) -> str:
    """Read the indented metadata.version value used by workflow skills."""
...
def main() -> int:
    ...
    for spec in OFFICIAL_SKILLS:
        ...
        package_version = frontmatter_metadata_version(md.decode("utf-8", "replace"))
        if package_version and package_version != spec.version:
            failures.append(
                f"{spec.key}: SKILL.md metadata.version {package_version!r} != registry {spec.version!r}"
            )
        if not spec.trigger_phrases:
            failures.append(f"{spec.key}: registry has no trigger_phrases")
        if not spec.required_tools:
            failures.append(f"{spec.key}: registry has no required_tools")
        overlap = set(spec.trigger_phrases) & set(spec.negative_phrases)
        if overlap:
            failures.append(
                f"{spec.key}: trigger/negative phrase overlap: {sorted(overlap)!r}"
            )
        for rel, data in files.items():
            if len(data) > MAX_SKILL_FILE_BYTES:
                failures.append(f"{spec.key}: {rel} exceeds {MAX_SKILL_FILE_BYTES}")
            if not is_official_skill_package_path(rel):
                failures.append(f"{spec.key}: unexpected package path {rel!r}")
        scripts = [
            rel
            for rel in files
