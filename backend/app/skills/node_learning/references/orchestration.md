...
**两段式强度（关键）**：
- **首次学习（全力形态）**：
  - 视觉维度一律优先 **L1 自绘 SVG**（结构/流程/关系/对比图）；仅真实照片/实物才用 L6 图片，数值对比才用 L2 图表。
  - 简单节点：L0 + L1 SVG + L3 交互练习——至少 3 档。
  - 中等节点：L0 + L1 SVG（有数值再加 L2）+ L3 交互练习 + 可选 L6 配图。
  - 复杂节点：L0 骨架 + L4/L5（主演示，内嵌 SVG 步骤图）+ L3 交互练习 + 可选 L6；L7 仅在用户要求时追加。
  - **交互练习：首次学习尽量配 1～3 题**（channel A 判分组件；多题连发自动堆叠成分页练习）。
- **追问 / 补充（收缩形态）**：先判断问题大小——小问题直接 L0 回答；需要图、演示或自测再临时升档；不重复首次学习的全维度讲解。
...
- **力度**：首次学习默认至少配 1 张图（按矩阵选来源）；追问阶段不主动加图，用户要才加。

## 3. 场景模板

> 以下模板均为**首次学习的全力形态**（默认用足 3～4 档）。其中视觉一律优先 **L1 自绘 SVG**，并**尽量配 L3 交互练习（1～3 题）**。用户追问、表示已懂或要求简要时，只保留与问题直接相关的 1～2 步即可，不要重复全量教学。
...
**首次学习尽量配交互练习**：默认 L3 判分题 1～3 道 +（过程/推导型）L4 交互演示 1 个；追问阶段按需，不重复。
...
> **首次学习的视觉一律手写 SVG（L1）**：结构/流程/概念/对比图默认用 SVG，不调用生图/搜图；只有真实照片、实物场景才走 L6。
...
**两段式决策法**：
- 首次学习：默认全力（3～4 档），视觉用 L1 SVG、尽量配 L3 交互练习，先说明投入；成本上限由"护栏"兜底，不为炫技加维度。
- 追问阶段：先判断问题大小——小问题用 L0 直接回答；需要演示、配图或自测再按阶梯临时升档。

## 7. 提交前自检清单

- [ ] 首次学习视觉维度是否一律用了 L1 自绘 SVG（结构/流程/概念图没让生图/搜图代劳）？
- [ ] 首次学习是否配有 L3 交互练习（1～3 题判分组件）？追问阶段是否已收缩到够用档位？
- [ ] 生图是否超预算（≤1 张/节点、≤2 张/会话）？是否优先用了引用与搜图？
