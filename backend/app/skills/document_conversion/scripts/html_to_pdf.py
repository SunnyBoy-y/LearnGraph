#!/usr/bin/env python3
"""Render a local HTML file to PDF with the image's headless Chromium (offline).

Thin CLI wrapper over ``learngraph_tasks.html_to_pdf``. The HTML must be
self-contained; external resources are skipped because the sandbox is offline.
"""
...
from learngraph_tasks import html_to_pdf
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Render a local HTML file to PDF (offline Chromium).")
