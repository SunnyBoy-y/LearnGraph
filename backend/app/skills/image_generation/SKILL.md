...
description: 独立图片生成或编辑：用宿主 generate_image 完成文生图与图生图，先解析会话图片 file_id，再生成或基于原图编辑，并区分搜图与生图。节点讲解中的结构图/流程图优先由 node-learning 自绘 SVG。
---
...
>
> **路由边界**：用户只是在节点教学中要结构图、流程图或概念关系图时，不使用本 Skill，交给 `node-learning` 的自绘 SVG；只有要独立生成/编辑图片，或节点教学确实需要照片/插画时才使用。
