...
5. **分层生成规则（强制）**：
   - **首次生成（create）**：流式策略为**先主干、再逐主干节点分层展开**——先出 0 层（唯一 root），再出 1 层主干（root 的直接 `contains` 子节点，构成骨架），随后**每个主干节点展开两层分层**（layer=1 直接子节点 + layer=2 孙节点）；禁止出现 depth>3 的更深层级，禁止孤儿节点；除 root 外每个节点恰好一条 `contains` 边挂到其父级。
...
6. **提交前自我校验清单**（全部通过后再调用工具；任一项失败则修正提案，不要把坏结构发给用户审核）：
   ...
   - 新建图谱至少 2 个 `add` 节点：0 层唯一 root + 1 层主干，且每个主干节点只展开两层分层（最多 depth 3）；更新提案只包含真正需要变更的节点/边，且新增节点只挂在已有父节点下。
...
- 「极速 / 思考」两种生成模式都支持流式：先出根节点预览，再出主干（1 层骨架），随后**逐主干节点展开两层分层**（layer=1 子节点 → layer=2 孙节点）；极速模式主干更紧凑（3-4 个）、分支更窄，思考模式主干更全（5-6 个）、分支更宽。
- 智能体路径的提案在生成完成后以**图谱提案审核卡**出现在消息流中，用户确认后才写入正式图谱；智能体不得声称已发布。
